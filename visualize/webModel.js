...
class Layer {
    ...
    activationDerivative(x) {
        if (this.activation === 'sigmoid') return x * (1 - x);
...
class NeuralNetwork {
    ...
    drawNN(canvasSelector, ignoreInput = false, cwidth = 600, cheight = 400, weightLineWidth = 0.5, nodeRadius = 6) {
        ...
        if (ignoreInput) {
            layerCount = this.numLayers;
        } else {
            layerCount = this.numLayers + 1;
            layerSizes.push(this.layers[0].numInputs);
        }
        this.layers.forEach(layer => layerSizes.push(layer.numNeurons));

        for (let l = 0; l < layerCount; l++) {
            const numNeurons = layerSizes[l];
            const layerX = (width / (layerCount - 1)) * l + width / 40;
            const neuronSpacing = height / (numNeurons + 1);

            const positions = [];
            for (let n = 0; n < numNeurons; n++) {
                const neuronY = neuronSpacing * (n + 1) + height / 40;
